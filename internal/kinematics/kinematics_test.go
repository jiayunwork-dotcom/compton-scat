...
// TestThetaZero checks the forward scattering limit: no wavelength
// shift, no energy transfer.
...
// TestTheta180 checks the backscatter limit: the shift reaches
// 2*lambda_c and the scattered energy is minimal.
...
// TestNinetyDegreeClosedForm checks the canonical 511 keV / 90 degree
// case against the closed form E' = E/(1+E/me*c^2).
...
// TestEnergyConservation checks Ke + E' == E for several energies and
// angles.
...
// TestWavelengthConsistency checks lambda' = lambda + dlambda and
// E' = h*c/lambda' for a solved event.
...
// TestMomentumConservation checks that the recoil kinetic energy from
// the momentum picture agrees with the energy formula.
...
// TestInvalidEnergy checks that zero and negative energies are rejected
// with an error, never silently solved.
...
// TestInvalidAngle checks that angles outside [0, 180] degrees are
// rejected.
...
// TestRelativeLossIncreases checks the cross rule that raising the
// energy at a fixed angle increases the relative loss.
...
// TestHighEnergyLargeAngle checks that a high-energy backscatter keeps
// a positive scattered energy and matches the closed form. A formula
// with a minus sign in the denominator produces a negative E' here.
...
// TestScatteredEnergyHelper checks the standalone helper returns the
// same value as the full solver.
...
// TestRunAllChecks verifies that the full check suite passes on a
// valid event and fails (or rejects) an invalid one.
