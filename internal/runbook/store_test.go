...
func TestSaveLoadRoundTrip(t *testing.T) {
	...
	b := NewBook(12)
	if err := b.Add(Entry{ID: "a", EnergyKEV: 511, AngleDeg: 90, ScatteredKEV: 255.5, RecoilKEV: 255.5}); err != nil {
		t.Fatal(err)
	}
...
func TestLoadRejectsCorruptSnapshot(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"version":3,"max":4,"seq":1,"entries":[]}`), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestImportRebuildsBook(t *testing.T) {
	b := NewBook(10)
	if err := b.Add(Entry{ID: "x", EnergyKEV: 100, AngleDeg: 60, ScatteredKEV: 80, RecoilKEV: 20}); err != nil {
		t.Fatal(err)
	}
	other := NewBook(3)
